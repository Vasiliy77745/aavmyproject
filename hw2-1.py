...
def format_with_fstring(data: Metrics):
    #return f"{f"CPU #1: {data.cpu_data[1]}%, Memory used: {data.memory_used}, Load avg: {round(data.load_avg, 2)}":*^64}"
    var = f"CPU #1: {data.cpu_data[1]}%, Memory used: {data.memory_used}, Load avg: {round(data.load_avg, 2)}"
    return f"{var:*^64}"
    #return f"CPU #1: {data.cpu_data[1]}%, Memory used: {data.memory_used}, Load avg: {round(data.load_avg, 2)}".center(
     #   64, "*")
...
def format_with_format(data: Metrics):
    ...
    #return "[{a}] CPU #7: {b}%, Memory used: {c}, Load avg: {d}".format(a=pid, b=data.cpu_data[7], c=data.memory_used,
      #                                                                  d=round(data.load_avg, 2)).rjust(64, '-')
...
def format_with_percent(data: Metrics):
    ...
    return "%64s" % f_data
    #f_data = "[0x%x] CPU #7: %d%%, Memory used: %d, Load avg: %.02f" % (data.agent_address, data.cpu_data[7], data.memory_used, data.load_avg)
    #return "%64s" % ("[0x%x] CPU #7: %d%%, Memory used: %d, Load avg: %.02f" % (data.agent_address, data.cpu_data[7], data.memory_used, data.load_avg))
