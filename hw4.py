...
class InfiniteGeneratorWrapper:
    ...
    def __init__(self, g: Generator[str, None, None], max_elements=1000) -> None:
        self.g = g
        self.max_elements = max_elements# insert code here
        self.index = 0
    def __iter__(self):
        return self
    def __next__(self):
        if self.index < self.max_elements:
            self.index += 1
            return binascii.hexlify("".join(list(reversed(next(self.g)))).encode("utf-8")).decode()
        else:
            raise StopIteration
        ...
        #for _ in range(self.max_elements):
            #a = next(self.g)
            #b = a[::-1]
            #c = binascii.hexlify(b.encode())
            #d = binascii.hexlify(c.decode())
            #l.append(c)
        #return c
        #return next(iter(l))

        #it = iter([binascii.hexlify("".join(list(reversed(next(self.g)))).encode("utf-8")) for _ in range(self.max_elements)])
        #it = iter([binascii.hexlify(next(self.g)[::1].encode("utf-8")) for _ in range(self.max_elements)])
        #return next(it)
...
class KeepEverythingStartsFrom(BaseRule):
    ...
    def check(self, item) -> bool:
        if item.startswith(self.starts_from):
            ...
            return self.deny(self.deny_reason)
